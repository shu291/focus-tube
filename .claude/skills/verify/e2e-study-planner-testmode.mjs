...
const LS = () => page.evaluate(() => ({
  ...
  testExists: localStorage.getItem('studyPlanner.test') !== null,
...
console.log('\n[H] 「まっさらに消す」→ テストの中身だけ空・領域は残る・本番は無傷');
await page.locator('.tabbtn', { hasText: '設定' }).click();
page.once('dialog', d => d.accept());
await page.locator('#tClear').click();
await page.waitForTimeout(300);
ls = await LS();
ok(ls.test === 0 && ls.testExists && ls.real === 1, 'テスト空・領域は存在・本番1 実際:' + JSON.stringify(ls));
ok(await page.locator('#testBanner').isVisible(), 'まっさらにしてもテストモードは継続');

console.log('\n[I] サンプルを入れ直して「全消去して終了」→ テスト領域ごと削除・本番へ');
await page.locator('#tSeed3').click();
await page.waitForTimeout(300);
ok((await LS()).test === 3, '3科目を入れ直し(test=3)');
page.once('dialog', d => d.accept());
await page.locator('#tDiscard').click();
await page.waitForTimeout(300);
ls = await LS();
ok(!ls.testExists && ls.mode === 'real' && ls.real === 1, 'テスト領域が消え・realモード・本番1 実際:' + JSON.stringify(ls));
ok(await page.locator('#testBanner').isHidden(), 'バナーが消える');

console.log('\n[J] もう一度入る → 本番の最新コピーから作り直される');
await page.locator('#tEnter').click();
await page.waitForTimeout(300);
ls = await LS();
ok(ls.testExists && ls.test === 1 && ls.real === 1, '本番コピーで再生成(test=1) 実際:' + JSON.stringify(ls));
