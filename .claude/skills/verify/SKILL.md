...
- `node e2e-study-planner-testmode.mjs`(25チェック)。本番 `studyPlanner.v1` を触らずに
  `studyPlanner.test` のコピーで試せることを検証:テスト突入で本番保持・コピー生成、
  **仮の今日を+5日にするとノルマが 10→17 に再計算**、サンプル投入はサンドボックスのみ増加、
  テスト中はGitHub APIが呼ばれない(`ctx.route` でヒット数0を確認)、本番復帰でノルマが実日付に戻る、
  リロードでモード維持。削除系は「まっさらに消す」(中身だけ空・領域は残る)と
  「全消去して終了」(`TEST_KEY` ごと `removeItem`・real復帰)を区別して検証し、
  再突入で本番コピーから作り直されること・本番書名が終始不変なことまで確認
- 日付比較は TZ ずれを避けるため、`realToday` はページ側 `toLocaleDateString('sv')` で取得し、
  `+N日` は node 側で `Date.UTC` ベースに計算する(ローカルTZの `new Date()` 加算はDSTで危険)
- 逆算は `todayKey()`(=`FAKE_TODAY || dayKey()`)一箇所で仮の今日に差し替わる。`new Date()` を
  直接使う箇所(ヒーローの日付・`defaultDeadline`)は `keyToDate(todayKey())` に寄せてある
